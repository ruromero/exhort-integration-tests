...
def validate_analysis(output: Dict[str, Any], spec: Dict[str, Any], analysis_type: str) -> bool:
    ...
    # Deterministic: component analysis always has transitive == 0
    ...
    # Minimum transitive threshold (for stack analysis)
    if "transitive_min" in exp_scanned:
        if scanned.get("transitive", 0) < exp_scanned["transitive_min"]:
            print(f"  FAIL {analysis_type} scanned.transitive ({scanned.get('transitive')}) "
                  f"< minimum {exp_scanned['transitive_min']}")
            ok = False
